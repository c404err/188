    ...
    def elapseTime(self, gameState):
        ...
        for pos in self.allPositions:
            ...
            for newpos in newPosDist:
                discr[newpos] += self.beliefs[pos] * newPosDist[newpos]
        discr.normalize()
        self.beliefs = discr
...
class ParticleFilter(InferenceModule):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for p in self.particles:
            pacmanPosition = gameState.getPacmanPosition()
            jail = self.getJailPosition()
            beliefs[p] += self.getObservationProb(observation, pacmanPosition, p, jail)
        beliefs.normalize()
        if beliefs.total() == 0:
            self.initializeUniformly(gameState)
        else:
            for i in range(self.numParticles):
                newPar.append(beliefs.sample())
            self.particles = newPar
    ...
    def elapseTime(self, gameState):
        ...
        "*** YOUR CODE HERE ***"
        newPar = list()
        for oldPar in self.particles:
            dist = self.getPositionDistribution(gameState, oldPar)
            newPar.append(dist.sample())
        self.particles = newPar
...
class JointParticleFilter(ParticleFilter):
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particles:
            ...
            for i in range(self.numGhosts):
                dist = self.getPositionDistribution(gameState, newParticle, i, self.ghostAgents[i])
